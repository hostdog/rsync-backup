...
# SYNC with IP
ionice -c3 rsync -ahv --log-file=/var/log/backups/${server}_rsync.log --delete --link-dest=../$last_backup --exclude-from=backup_exclude --rsh="/usr/bin/ssh ${ssh_port}" root@${server_ip}:/ temp_rsync >> /var/log/backups/${server}_stout.log && mv temp_rsync day_${today}
